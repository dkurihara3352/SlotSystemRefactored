	...
	namespace SlotGroupTests{
		...
		public class SlotGroupTests: SlotSystemTest{
			...
			/*	sse override	*/
				...
				public void FocusSBs_Various_CallsSBFocusOrDefocus(List<ISlottable> sbs, Dictionary<IUIElement, bool> dict){
					SlotGroup sg = MakeSG_ISBHandler(sbs);
					...
					foreach(var e in sg){
						if(dict[e])
							e.Received().MakeSelectable();
						else
							e.Received().MakeUnselectable();
					}
				}
					class FocusSBsVariousCases: IEnumerable{
						public IEnumerator GetEnumerator(){
							...
							Dictionary<IUIElement, bool> dict = new Dictionary<IUIElement, bool>();
								dict.Add(fSBA, true);
								dict.Add(fSBB, true);
								dict.Add(fSBC, true);
								dict.Add(dSBA, false);
				...
				public void InspectorSetUp_Always_SetsFields(int initSlotsCount){
					SlotGroup sg = MakeSG();
						ISGToolHandler sgToolHandler = Substitute.For<ISGToolHandler>();
				...
				class SGSetUpFieldsData: IEquatable<SGSetUpFieldsData>{
					...
					public UIState sgSelState;
					public UIState sgActState;
					public SGSetUpFieldsData(string name, SGFilter filter, IInventory inventory, ISGCommand OnACompComm, ISGCommand oAExecComm, bool isShrinkable, int initSlotsCount, bool isExpandable, UIState sgSelState, UIState sgActState){
						this.name = name;
						this.filter = filter;
			...
			/* Transaction */
				...
				public void InitSBs_IsNotExpandableAndSlotsNotEnough_ThrowsException([NUnit.Framework.Random(2, 10, 1)]int itemCount){
					...
					sg.InspectorSetUp(Substitute.For<ISGToolHandler>(), Substitute.For<IInventory>(), Substitute.For<SGFilter>(), Substitute.For<SGSorter>(), itemCount -1);
				...
				public void InitSBs_SlotsAreReady_CreateSBsAndPutThemInSlots([NUnit.Framework.Random(1, 10, 1)]int itemsCount){
					...
					sg.InspectorSetUp(Substitute.For<ISGToolHandler>(), Substitute.For<IInventory>(), Substitute.For<SGFilter>(), Substitute.For<SGSorter>(), 0);
			...
			/*	helper */
				...
				static ISlottable MakeSBWithActProc(bool isRunning){
					ISlottable sb = MakeSubSB();
						IUIProcess sbActProc = MakeSubSBActProc();
