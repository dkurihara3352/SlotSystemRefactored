...
using System;
namespace UISystem{
	public class SlotGroup : ISlotSystemElement, ISlotGroup{
		...
		public ISGToolHandler GetToolHandler(){
			Debug.Assert(_toolHandler != null);
			return _toolHandler;
		}
			ISGToolHandler _toolHandler;
		void SetToolHandler(ISGToolHandler handler){
		...
		/*	intrinsic */
			...
			public void InspectorSetUp(ISGToolHandler toolHandler, IInventory inv, SGFilter filter, SGSorter sorter, int initSlotsCount){
	...
	public interface ISlotGroup: IUIElement{
			ISGToolHandler GetToolHandler();
		...
		/*	instrinsic	*/
			...
			void OnInventoryUpdate(object source, IInventory inventory);
