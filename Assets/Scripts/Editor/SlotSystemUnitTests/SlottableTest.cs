		...
		public void TestSGECorrespondence(){
			...
			ssm.PointFocus(sgpWear);
				...
				foreach(Slottable sb in transactableSBs(sgp, sge, typeof(SwapTransaction))){
					// Print(sb);
					InventoryItemInstance testItem = sb.itemInst;
					InventoryItemInstance swapItem = ssm.GetTransaction(sb, sge).targetSB.itemInst;
					AssertFocused();
					Swap(sb, sgp, sge);
				...
				foreach(Slottable sbe in sge){
					if(sbe != null){
						foreach(Slottable sbp in transactableSBs(sgp, sbe, typeof(SwapTransaction))){
							InventoryItemInstance testItem = sbp.itemInst;
							InventoryItemInstance swapItem = sbe.itemInst;
		...
		public void TestSwapOnAll(){
			...
			public void TestSwap(SlotGroup tarSG, Slottable testSB, bool isPAS, bool isTAS){
				...
				if(origSG != null)
					...
					origSG = origSGCache;
				InventoryItemInstance testItem = testSB.itemInst;
				...
				if(testSB.isFocused){
					...
					if(ta is SwapTransaction){
						...
						InventoryItemInstance swapItem = ta.targetSB.itemInst;
					...
					foreach(Slottable tarSB in tarSG){
						if(tarSB != null){
							...
							if(ta2 is SwapTransaction){
								...
								InventoryItemInstance tarItem = tarSB.itemInst;
				...
				foreach(Slottable tarSB in tarSG){
					if(tarSB != null){
						...
						if(testSB.isFocused){
							...
							if(ta.GetType() == typeof(SwapTransaction)){
								Capture(ssm, testSB, tarSB, isPAS, isTAS, TestElement.SB);
								InventoryItemInstance swapItem = tarSB.itemInst;
								Swap(testSB, origSG, tarSB);
								/*	reverse	*/
			...
			public void AssertEquipped(InventoryItemInstance itemInst){
				if(itemInst is BowInstance || itemInst is WearInstance){
					...
					foreach(Slottable sbp in sgpAll){
						if(sbp != null){
							InventoryItemInstance sbpItem = sbp.itemInst;
