...
public class EquipInventoryTests {
    /*  Add */
        ...
        public void Add_BowInst_EquipsBow(IEnumerable<BowInstance> added, BowInstance expected){
            ...
            class AddBowCases: IEnumerable{
                public IEnumerator GetEnumerator(){
                    BowInstance bowInst_A_1 = MakeBowInst(0);
                    BowInstance bowInst_A_2 = MakeBowInst(0);
                    BowInstance bowInst_B_1 = MakeBowInst(1);
                    BowInstance bowInst_B_2 = MakeBowInst(1);
        ...
        public void Add_WearInst_EquipsWear(IEnumerable<WearInstance> added, WearInstance expected){
            ...
            class AddWearInstCases: IEnumerable{
                public IEnumerator GetEnumerator(){
                    WearInstance wearInst_A_1 = MakeWearInst(0);
                    WearInstance wearInst_A_2 = MakeWearInst(0);
                    WearInstance wearInst_B_1 = MakeWearInst(1);
                    WearInstance wearInst_B_2 = MakeWearInst(1);
        ...
        public void Add_CGearsNotExceedingLimit_EquipCGears(int equippableCount, IEnumerable<CarriedGearInstance> added, IEnumerable<CarriedGearInstance> expected){
            ...
            class AddCGearsNotExceedingLimitCases: IEnumerable{
                public IEnumerator GetEnumerator(){
                    ShieldInstance shieldInst_A = MakeShieldInst(0);
                    MeleeWeaponInstance mWeaponInst_A = MakeMeleeWeaponInst(0);
                    QuiverInstance quiverInst_A = MakeQuiverInst(0);
                    PackInstance packInst_A = MakePackInst(0);
        ...
        public void Add_CGearsExceedingLimit_ThrowsException(List<CarriedGearInstance> fitted, CarriedGearInstance exceeded){
            ...
            class AddCGExeedingCases: IEnumerable{
                public IEnumerator GetEnumerator(){
                    ShieldInstance shield_A = MakeShieldInst(0);
                    ShieldInstance shield_A_2 = MakeShieldInst(0);
                    MeleeWeaponInstance mWeapon_A = MakeMeleeWeaponInst(0);
                    QuiverInstance quiver_A = MakeQuiverInst(0);
                    PackInstance pack_A = MakePackInst(0);
    ...
    /*  Remove  */
        ...
        public void Remove_NonMember_IgnoresUpdate(List<InventoryItemInstance> added, IEnumerable<InventoryItemInstance> removed){
            ...
            class RemoveNonMemberCases: IEnumerable{
                public IEnumerator GetEnumerator(){
                    BowInstance bow_A = MakeBowInst(0);
                    BowInstance bow_A_1 = MakeBowInst(0);
                    WearInstance wear_A = MakeWearInst(0);
                    WearInstance wear_A_1 = MakeWearInst(0);
                    ShieldInstance shield_A = MakeShieldInst(0);
                    ShieldInstance shield_A_1 = MakeShieldInst(0);
                    MeleeWeaponInstance mWeapon_A = MakeMeleeWeaponInst(0);
                    QuiverInstance quiver_A = MakeQuiverInst(0);
                    PackInstance pack_A = MakePackInst(0);
    ...
    /*  Helpers */
        ...
        private static BowInstance MakeBowInst(int id){
        ...
        private static WearInstance MakeWearInst(int id){
            WearFake wearFake = new WearFake();
            wearFake.ItemID = 100 + id;
            WearInstance wearInst = new WearInstance();
            wearInst.Item = wearFake;
            return wearInst;
        }
        private static ShieldInstance MakeShieldInst(int id){
            ShieldFake shieldFake = new ShieldFake();
            shieldFake.ItemID = 200 + id;
            ShieldInstance shieldInst = new ShieldInstance();
            shieldInst.Item = shieldFake;
            return shieldInst;
        }
        private static MeleeWeaponInstance MakeMeleeWeaponInst(int id){
            MeleeWeaponFake mWeaponFake = new MeleeWeaponFake();
            mWeaponFake.ItemID = 300 + id;
            MeleeWeaponInstance mWeaponInst = new MeleeWeaponInstance();
            mWeaponInst.Item = mWeaponFake;
            return mWeaponInst;
        }
        private static QuiverInstance MakeQuiverInst(int id){
            QuiverFake quiverFake = new QuiverFake();
            quiverFake.ItemID = 400 + id;
            QuiverInstance quiverInst = new QuiverInstance();
            quiverInst.Item = quiverFake;
            return quiverInst;
        }
        private static PackInstance MakePackInst(int id){
            PackFake packFake = new PackFake();
            packFake.ItemID = 500 + id;
            PackInstance packInst = new PackInstance();
            packInst.Item = packFake;
            return packInst;
        }
        private static PartsInstance MakePartsInst(int id){
            PartsFake partsFake = new PartsFake();
            partsFake.ItemID = 600 + id;
