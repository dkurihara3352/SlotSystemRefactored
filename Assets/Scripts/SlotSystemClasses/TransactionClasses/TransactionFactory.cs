...
namespace SlotSystem{
	public class TransactionFactory: ITransactionFactory{
		...
		public TransactionFactory(ITransactionManager tam, ITransactionIconHandler iconHandler, ITAMActStateHandler tamStateHandler){
			this.tam = tam;
					...
					if(hovSG.AcceptsFilter(pickedSB)){
						if(hovSG != origSG && origSG.isShrinkable){
							if(hovSG.HasItem(pickedSB.item) && pickedSB.item.Item.IsStackable)
								return new StackTransaction(pickedSB, hovSG.GetSB(pickedSB.item), tam, iconHandler, tamStateHandler);
								
							if(hovSG.hasEmptySlot){
								if(!hovSG.HasItem(pickedSB.item))
									return new FillTransaction(pickedSB, hovSG, tam, iconHandler, tamStateHandler);
							}else{
								if(hovSG.isExpandable){
									...
									if(hovSG.SwappableSBs(pickedSB).Count == 1){
										...
										if(calcedSB.item != pickedSB.item)
											return new SwapTransaction(pickedSB, calcedSB, tam, iconHandler, tamStateHandler);
									}
								}
							}
						}
					}
					return new RevertTransaction(pickedSB, tam, iconHandler, tamStateHandler);
					...
					if(hovSBSG == origSG){
						if(hovSB != pickedSB){
							if(!hovSBSG.isAutoSort)
								return new ReorderTransaction(pickedSB, hovSB, tam, iconHandler, tamStateHandler);
						}
					}else{
						if(hovSBSG.AcceptsFilter(pickedSB)){
							...
							if(pickedSB.item == hovSB.item){
								if(hovSBSG.isPool && origSG.isShrinkable)
									return new FillTransaction(pickedSB, hovSBSG, tam, iconHandler, tamStateHandler);
								if(pickedSB.item.Item.IsStackable)
									return new StackTransaction(pickedSB, hovSB, tam, iconHandler, tamStateHandler);
							}else{
								if(hovSBSG.HasItem(pickedSB.item)){
									if(!origSG.HasItem(hovSB.item)){
										if(hovSBSG.isPool){
											if(origSG.AcceptsFilter(hovSB))
												return new SwapTransaction(pickedSB, hovSB, tam, iconHandler, tamStateHandler);
